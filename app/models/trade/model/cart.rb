    ...
    class_methods do
      ...
      def get_cart(params, current_cart_id: nil, **options)
        ...
        if cart_id
          ...
          options.with_defaults! params.permit(:desk_id, :station_id, :contact_id, :client_id).to_h.to_options # 合并来自 params 的参数，转化为 symbol key
          ...
          options.with_defaults! ([:agent_id, :station_id, :desk_id, :client_id, :contact_id] & column_names.map(&:to_sym)).each_with_object({}) { |i, h| h.merge! i => nil }
