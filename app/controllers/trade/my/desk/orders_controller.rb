module Trade
  class My::Desk::OrdersController < Admin::Desk::OrdersController
    ...
    def index
      ...
      @order_s = Order.where(user_id: [nil, current_user.id]).default_where(q_params)
    ...
    def confirm
      ...
      @orders = Order.where(user_id: nil).default_where(q_params)
      @orders.update(user_id: current_user.id)
    end
